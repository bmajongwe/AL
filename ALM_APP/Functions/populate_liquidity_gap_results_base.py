...
def populate_liquidity_gap_results_base(fic_mis_date, process_name):
    ...
    try:
        # Step 1: Delete existing records in LiquidityGapResultsBase for the same fic_mis_date and process_name
    ...
    def process_cashflow_group(grouped_cashflow):
        ...
        # Step 6: Iterate over the unique 4 time buckets to populate records
        for idx, bucket in enumerate(time_buckets):
            inflows = grouped_cashflow.get(f'bucket_{idx+1}', 0) if flow_type == 'Inflow' else 0
            outflows = grouped_cashflow.get(f'bucket_{idx+1}', 0) if flow_type == 'Outflow' else 0
            ...
            # Check if this record already exists to avoid duplication
            ...
            try:
                LiquidityGapResultsBase.objects.create(
                    ...
                    n_total_cash_flow_amount=grouped_cashflow.get(f'bucket_{idx+1}', 0),
    ...
    # Step 7: Process each cashflow group in parallel
    with ThreadPoolExecutor() as executor:
        for element in cashflow_data.values():
            executor.map(process_cashflow_group, element)
